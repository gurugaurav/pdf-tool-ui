...
const Page = () => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      const response = await fetch('/api/pdf_to_image', {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-4 ">
      <h1 className="text-3xl font-bold mb-4 text-slate-800">PDF to Images</h1>
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-md w-full max-w-md mt-4">
        ...
        <button 
          type="submit" 
          className="w-full px-4 py-2 bg-blue-500 text-white font-semibold rounded-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"  disabled={loading}  >
